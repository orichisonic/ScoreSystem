  ...
  <div>
     <ul style="text-align:left">
       <li>大区<select name="zone" id="zone">
           ...
           <select id="area"><option value="">默认</option></select>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        mygrid = $("#list").jqGrid({
            width: 800,
            height: 450,
            url: 'http://' + location.host + '/ActivityRecommendedInterface.aspx/LoadRecommendedSubmit/',
            //            editurl: 'http://' + location.host + '/ActivityRecommendedInterface.aspx/LoadRecommendedSubmit/',
            datatype: 'json',
            mtype: 'POST',
            pager: jQuery('#pager'),
            rowNum: 20,
            ...
            onSelectRow: function(id) {
                ...
                if ($('#' + id + "_importbool").attr('checked') == true) {
                    ...
                    COUNTRY = COUNTRY + ";" + mygrid.getRowData(id).county + ";" + ",";
                    BARNAME = BARNAME + ";" + mygrid.getRowData(id).barname+ ";" + ",";
                    ADDRESS = ADDRESS + ";" + mygrid.getRowData(id).address + ";" + ",";
                    tmp=mygrid.getRowData(id).premium;
                    if (tmp.indexOf('value=') >= 0)
                        PREMIUM = PREMIUM + ";" + mygrid.getRowData(id).premium.substring(mygrid.getRowData(id).premium.indexOf('value=') + 6, mygrid.getRowData(id).premium.indexOf(">")) + ";" + ",";
                    else
                        PREMIUM = PREMIUM + ";" +"" + ";" + ",";
                }
                else {
                    if (selectedRows.indexOf(";" + mygrid.getRowData(id).netbarid + ";") >= 0) {
                        ...
                        COUNTRY = COUNTRY.replace(";" + mygrid.getRowData(id).county + ";" + ",", "");
                        BARNAME = BARNAME.replace(";" + mygrid.getRowData(id).barname + ";" + ",", "");
                        ADDRESS = ADDRESS.replace(";" + mygrid.getRowData(id).address + ";" + ",", "");
                        tmp=mygrid.getRowData(id).premium;
                        if (tmp.indexOf('value=') >= 0)
                        PREMIUM = PREMIUM.replace(";" + mygrid.getRowData(id).premium.substring(mygrid.getRowData(id).premium.indexOf('value=') + 6, mygrid.getRowData(id).premium.indexOf(">")) + ";" + ",", "");
                        else
...
//    }
    ...
    function submitInfo() {
        ...
        var countryid = COUNTRY.replace(new RegExp(';', "gm"), '');
        countryid = countryid.substr(0, countryid.length - 1);
        ...
        var premiumid = PREMIUM.replace(new RegExp(';', "gm"), '');
        premiumid = premiumid.substr(0, premiumid.length - 1);
        premiumid = premiumid.split(",");
        ...
        //发送参数保存数据
         ...
         for (var i = 0; i < provid.length; i++) {
             ...
             json_data = { AREAID: $("#area").val(), PROV: provid[i], CITY: cityid[i], COUNTRY: countryid[i], BARNAME: barnameid[i], ADDRESS: addressid[i], PREMIUM: premiumid[i] };

            
            $.ajax({
            url: "http://" + location.host + "/ActivityRecommendedInterface.aspx/SubmitRemmendedBar/",
                type: "POST",
                dataType: "json",
                data: json_data,
    ...
    function init() {
        ...
        if ($("#msg").text() == "活动提交成功！") {
            ...
            $('#msg5').text("");
            $('#msg6').text("");
            var str = "http://" + location.host + "/ActivityRecommendedInterface.aspx/RecommendedSubmit/";
            location.reload(str);
        }
        else {
