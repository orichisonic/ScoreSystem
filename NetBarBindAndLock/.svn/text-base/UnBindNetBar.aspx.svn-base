...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <p style="text-align: center; color: Orange; font-size: 14px; font-weight: bold;">解绑日期:<%=DateTime.Now.ToLongDateString() %></p>
<fieldset>
    <legend>人员信息</legend>
    <div style="text-align:center">
        <ul class="horizontal">
            ...
            <%NetBarLockAndBind netbarInfo = (NetBarLockAndBind)ViewData["NetBarInfo"];
                ...
                <li>
                    绑定网吧数量: <span id="lockcount"><%=netbarInfo.BINDNUM%></span>
                </li>
                <li>
 ...
 <p style="text-align: center"><button id="submitBtn" class="fg-button ui-state-default ui-corner-left" type="button" onclick="submitInfo()">解绑</button></p>

<script language="javascript" type="text/javascript">
    var idStr = "";
    $(document).ready(function() {
        $("#user").change(function() { loadUserInfo(); });
        //$("#user").click(function() { loadUserInfo(); });
        arrestingUser();
    });

    function loadUserInfo() {
        var str = "http://" + location.host + "/NetBarBindAndLock.aspx/LoadUnBindUserInfo/" + $("#user").val();
